...
namespace PingPong3.Patterns.Builder
{
    class FrenzyLevelBuilder : LevelBuilder
    {
        ...
        public override void BuildMovingWalls()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                wall = wallFactory.MakeWall(1).SetData(new Point(792 - (30 * i), -90 + (30 * i)), new Size(30, 180), Color.White, -90, 90, new Point(0, 2));
                levelData.walls.Add(wall.SetMove(new MoveVertically(wall)));
                wall = wallFactory.MakeWall(1).SetData(new Point(232 + (30 * i), -90 + (30 * i)), new Size(30, 180), Color.White, -90, 90, new Point(0, 2));
                levelData.walls.Add(wall.SetMove(new MoveVertically(wall)));

                wall = wallFactory.MakeWall(1).SetData(new Point(612 + (30 * i), 678 + (30 * i)), new Size(30, 180), Color.White, 678, 858, new Point(0, 2));
                levelData.walls.Add(wall.SetMove(new MoveVertically(wall)));
                wall = wallFactory.MakeWall(1).SetData(new Point(412 - (30 * i), 678 + (30 * i)), new Size(30, 180), Color.White, 678, 858, new Point(0, 2));
                levelData.walls.Add(wall.SetMove(new MoveVertically(wall)));
            }
        }
